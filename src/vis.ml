  ...
  let rec ssubfs_cell_row row idx skip (f: Formula.t) (p: Expl.Proof.t) : (cell_row * int) =
    ...
    | S sp -> (match f, sp.Hashcons.node with
               ...
               | Prev (i, f'), SPrev sp
                 | Once (i, f'), SOnce (_, sp)
                 | Next (i, f'), SNext sp
                 | Eventually (i, f'), SEventually (_, sp) ->
                  let sp_idx = idx+1 in
                  let (row', idx') = ssubfs_cell_row row sp_idx false f' (S sp) in
                  ...
                  let idx' = if skip then idx'+1 else idx' in
                  ((cell, cells) :: row', idx')
               | Historically (_, _), SHistoricallyOut _ ->
                  let cell = (Expl.Proof.p_at p, idx, None, Boolean false) in
                  let idx = if skip then idx+1 else idx in
                  ((cell, []) :: row, idx)
               | Historically (i, f'), SHistorically (_, _, sps)
                 | Always (i, f'), SAlways (_, _, sps) ->
                  let sps_idx = idx+1 in
               ...
               | Since (i, f1, f2), SSince (sp2, sp1s)
                 | Until (i, f1, f2), SUntil (sp2, sp1s) when Fdeque.is_empty sp1s ->
                  let sp1_idx = idx+1 in
                  (* Recursive calls *)
                  let sp2_idx = (cell_idx sp1_idx f1)+1 in
                  let (row', idx') = ssubfs_cell_row row sp2_idx false f2 (S sp2) in
    ...
    | V vp -> (match f, vp.Hashcons.node with
               | FF, VFF _ ->
                  let cell = (Expl.Proof.p_at p, idx, None, Boolean false) in
                  let idx = if skip then idx+1 else idx in
                  ((cell, []) :: row, idx)
               | EqConst _, VEqConst _ ->
                  let cell = (Expl.Proof.p_at p, idx, None, Boolean false) in
                  let idx = if skip then idx+1 else idx in
                  ((cell, []) :: row, idx)
               | Predicate _, VPred _ ->
               ...
               | And (f1, f2), VAndR vp2 ->
                  ...
                  let idx' = if skip then idx'+1 else idx' in
                  ((cell, cells) :: row', idx')
               | Imp (f1, f2), VImp (sp1, vp2)
                 | Iff (f1, f2), VIffSV (sp1, vp2) ->
                  let sp1_idx = idx+1 in
                  let (row', idx') = ssubfs_cell_row row sp1_idx false f1 (S sp1) in
                  let vp2_idx = idx'+1 in
                  let (tbl'', idx'') = ssubfs_cell_row row' vp2_idx false f2 (V vp2) in
               ...
               | Forall (_, f'), VForall (x, d, vp) ->
                  ...
                  ((cell, cells) :: row', idx')
               | Prev (i, f'), VPrev vp
                 | Historically (i, f'), VHistorically (_, vp)
                 | Next (i, f'), VNext vp
                 | Always (i, f'), VAlways (_, vp) ->
                  let vp_idx = idx+1 in
                  let (row', idx') = ssubfs_cell_row row vp_idx false f' (V vp) in
                  let cell = match f with Prev _
                  ...
                  let idx' = if skip then idx'+1 else idx' in
                  ((cell, cells) :: row', idx')
               | Once (i, f'), VOnce (_, _, vps)
                 | Eventually (i, f'), VEventually (_, _, vps) ->
                  let vps_idx = idx+1 in
               ...
               | Since (i, f1, _), VSince (_, vp1, vp2s)
                 | Until (i, f1, _), VUntil (_, vp1, vp2s) when Fdeque.is_empty vp2s ->
                  let vp1_idx = idx+1 in
                  let (row', idx') = ssubfs_cell_row row vp1_idx false f1 (V vp1) in
